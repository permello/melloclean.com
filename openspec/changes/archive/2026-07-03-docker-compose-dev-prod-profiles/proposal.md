...
`docker-compose.yml` was emptied out when the repo moved from a 2-service (frontend/backend) layout to the current turborepo structure (`apps/client`, `apps/dashboard`, `apps/server`) and no longer reflects how the app should actually be run. The root `Dockerfile` already has multi-stage targets for both a shared `development` stage and per-app production stages (`client-production`, `dashboard-production`, `server-production`), and an `nginx/nginx.dev.conf` already routes nip.io subdomains to the three apps — but nothing wires these together into a runnable topology, and there's no equivalent for production. We want one `docker-compose.yml`, using Compose `profiles`, that can bring up either a dev topology or a prod topology, with both topologies structurally mirroring each other (three app containers + nginx) so dev behavior is representative of production.

The app's backend is Appwrite rather than a hand-rolled Postgres + Express data layer. `apps/server` (currently an empty Express stub) stays as a thin BFF that talks to Appwrite server-side — client and dashboard never call Appwrite directly. Appwrite itself is never a container in this repo: dev points at Appwrite Cloud (managed SaaS), prod points at a self-hosted Appwrite instance running on a separate VPS outside this compose topology. This keeps the compose footprint identical to a plain 3-app-plus-nginx topology in both profiles — no local resource budget is spent on Appwrite, and there's no vendoring/upgrade burden for a self-hosted stack in this repo.
...
- No `db`/Postgres service. Appwrite is the app's data/auth/storage backend, and it is not part of this compose file: `server`/`server-dev` reach it as an external HTTP dependency via `APPWRITE_ENDPOINT` / `APPWRITE_PROJECT_ID` / `APPWRITE_API_KEY` env vars, sourced from whichever `.env` is active for that deployment — a developer's local `.env` holds Appwrite Cloud values, the prod host's `.env` holds the self-hosted VPS's values. Provisioning and TLS for the self-hosted Appwrite VPS are out of scope for this change.
...
### New Capabilities
- `deployment-topology`: Defines how the app's containers (client, dashboard, server, nginx) are composed and networked for local development and for production, including which Dockerfile build targets and nginx configs apply to each, and how `server` is configured to reach the external Appwrite backend.
...
- `.env.example` (add `APPWRITE_ENDPOINT` / `APPWRITE_PROJECT_ID` / `APPWRITE_API_KEY`; remove `POSTGRES_*`)
- `apps/server` gains the Appwrite SDK as a dependency for its server-side BFF calls; no other application code changes; root `Dockerfile` targets are consumed as-is, not modified
...
- No self-hosted Appwrite provisioning, VPS setup, or TLS is in scope here — this change only wires `server`'s configuration to point at wherever Appwrite already lives
