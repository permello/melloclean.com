# workspace-typecheck-convention Specification

## Purpose
TBD - defines the convention that every package under `apps/*` and `packages/*` provides a `typecheck` script so the root `npm run typecheck` (Turbo) exercises every package instead of silently skipping any.
